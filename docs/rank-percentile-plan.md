...
> **2026-07-30 후속:** 통합 완료. `GET /api/stats`(`snapshot()`) + 포기 제출(`solved:false`)로
> TODO 6번이 닫혔다. 구현 중 알게 된 것 하나 — **추측 0번 포기를 허용해야 한다.** 첫 추측 전에
> 포기하는 사람이 `guesses >= 1` 검증에 걸려 400을 받으면 참여 집계에서 통째로 빠지고,
> 그만큼 정답률이 부풀려진다. 지금은 `solved=false`일 때만 0을 받는다.
