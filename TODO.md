...
> 현재 상태: 모노레포 5개 영역 모두 구현·검증 완료. data(15,218개 — 확장 제외) / packages/core(테스트 5/5) /
...
현재 규칙: `workers/api/src/answer.ts` `buildAnswerPool` — 종합 순위 `POOL_MAX_RANK`(5,500) 이내 +
weight/players 있고 태그≥2 + 평가 50+, 프랜차이즈 최상위 1개만, 상위 1200개.

- [x] **수록 범위를 보드라이프 등록 전체로 확대(2026-08-07)** — 5,407개 → 21,194개.
      `data/crawl_all.py`가 id를 전수 스윕한다(옛 `/rank/N` 목록 페이지는 사이트에서 없어짐).
      정답 풀은 의도적으로 넓히지 않았다: 예전에는 `rank != null`이 곧 "랭킹 목록권(상위 5,500)"이라
      1차 관문 역할을 했는데, 전 게임에 순위가 붙으면서 그 조건이 아무것도 못 거르게 됐다.
      `POOL_MAX_RANK`로 경계를 명시적으로 되살렸다. **실측 풀 565개(이전 551개)로 사실상 동일.**
- [x] 확장팩이 정답으로 새는 문제 — 보드라이프가 확장에는 순위를 안 매긴다는 걸 확인.
      전수 데이터의 `rank is None` 6,239개가 대부분 확장·프로모라 `rank != null`로 자동 배제된다.
      (baseName 분리 로직보다 이쪽이 훨씬 정확)
- [x] **정답 반복 문제 해결(2026-08-14)** — 원인은 풀 크기가 아니라 **뽑는 방식**이었다.
      날짜 해시로 매일 독립 추첨해서 365일 중 82일이 재출현했고 최단 간격이 5일이었다.
      사이클마다 풀을 섞어 순서대로 소진하도록 바꿔 한 사이클(565일) 무중복.
      자세한 내용은 [docs/code-audit-2026-08.md](./docs/code-audit-2026-08.md).
- [ ] **`baseName` 묶음이 다른 게임까지 합침 — bgg_id 기준으로 교체할 것 (565 → 615)**.
      탈락 56개 중 52개가 bgg_id가 다른 진짜 다른 게임이다(브라스: 랭커셔 r14,
      도미니언 r65, 듄: 임페리움 r68 …). 자격 게임 621개 전부 bgg_id를 갖고 있다.
      "같은 시리즈" 배지가 배포됐으므로 이제 안전하게 넓힐 수 있다 → 감사 문서 A 항목
- [ ] 너무 마이너/애매한 정답 추가 필터 검토 (평점 수, 인지도)
- [ ] 풀 크기(1200) 적정성 — 실측 풀은 565개라 limit이 걸린 적이 없다
...
- [x] **모바일 키보드가 입력창을 가리는 문제** (`lib/keyboard.ts`) — 키보드가 뜨면 보이는 영역
      (visualViewport)만 줄고 레이아웃 뷰포트는 그대로다. 브라우저가 입력창을 끌어올리려면
      **스크롤 여유**가 필요한데, 추측 전 페이지는 짧아서 그 여유가 없어 입력창이 키보드 밑에 남았다.
  - `useKeyboardInsets` — 키보드가 덮은 높이를 `--kb`로 내보내 `.wrap` 아래 여백에 더한다.
    딱 그만큼 스크롤 여유가 생긴다. 키보드가 없으면 0이라 평소 레이아웃은 그대로
  - `useScrollInputIntoView` — 보이는 영역 기준으로 입력창을 맨 위에 붙인다. 자동완성이
...
- [x] **자동완성 후보 8 → 30개** (`SUGGEST_LIMIT`) — 시리즈가 긴 검색어("브라스")에서 찾던 편이
      ...
      (`scrollIntoView`는 페이지까지 움직여서 직접 계산). 실측 "브라스" 8개 → 24개
- [ ] 모바일 입력/자동완성 키보드 동작 추가 다듬기
...
- 머더미스터리 **시나리오 키트 182개 제외**(정통 추리게임 유지). 제외 목록: `data/exclude_list.json`.
  전수 크롤로 키트류가 대거 유입돼 93 → 182로 늘었다. 본편(트래지디 루퍼·디텍티브 시리즈 등)은
  `build_exclude.py`의 KEEP 화이트리스트가 지킨다.
- **확장 5,981개 제외**(`data/expansion_list.json`). 판정은 보드라이프 카테고리 **55 = "본판이
  필요한 확장"** — 이미 크롤하는 필드라 추가 요청이 없고 실측 오탐 0이다. 55가 놓친 81건은
  이름 정규식(`확장|프로모|expansion|promo`)이 보강한다. `rank is None`은 대체 신호로 쓸 수
  없다(순위 없는 본판이 430개). 좀비사이드 시즌 2 같은 **스탠드얼론 확장은 55가 안 붙고 남는데
  의도한 동작**이다 — 혼자 놀 수 있으면 게임으로 취급한다.
  정답 풀은 이 제외로 안 바뀐다(566개 그대로) — `answer.ts`의 `rank <= 5500` + 프랜차이즈
  중복제거가 이미 확장을 전부 걸러내고 있었다. 줄어드는 건 추측·자동완성 대상이다.
- 데이터 재생성 절차는 README 참고. 중간 산출물(games_detail_all.json 등)은 .gitignore에 있음.
- ⚠️ **보드라이프 마크업은 바뀐다.** 2026-08 개편으로 세 군데가 한꺼번에 죽었다:
...
- 유형(types)은 이제 값이 **하나뿐**이고 이름도 짧아졌다("전략게임" → "전략"). 옛 `/info/type/`은
  복수 값이 가능했다.
- ⚠️ **플레이타임은 범위다**(약 40%가 "50-140분" 꼴). 옛 정규식 `(\d+)\s*분`이 앞쪽 50을 건너뛰고
  뒤쪽 140에 매칭해 **최댓값을 `time_min`이라는 이름으로** 저장했고, 힌트가 할러타우를
  "1-4명 · 140분"으로 잘못 보여줬다. 지금은 `time_min`/`time_max` 둘 다 저장한다(`patch_time.py`).
  **엔진 유사도는 계속 상한(`time_max`)을 쓴다** — 하한으로 바꾸면 기존 순위가 통째로 흔들린다
  (`packages/core/src/types.ts`의 `IndexedGame.timeMax` 주석).
