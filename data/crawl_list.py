"""[폐기] 보드라이프 순위 페이지(/rank/N)를 크롤링해 게임 명단을 수집한다.

⚠️ 2026-08 기준 `/rank/N`은 사라졌다(무한 스크롤로 개편). 그리고 애초에 이 목록은
등록 게임의 일부(5,500개)만 준다. 지금 파이프라인은 `crawl_all.py`가 id를 전수
스윕하며 상세 페이지에서 이름·연도·순위·평점까지 직접 뽑는다. 이 파일은 기록용.
